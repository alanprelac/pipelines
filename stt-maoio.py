...
import json
from pydantic import BaseModel  # Dodano za Valves

class Valves(BaseModel):  # Nasljeđuje BaseModel da ima model_dump()
    ASR_API_URL: str = "http://h1.maoio.eu:32772/asr"  # Koristi default vrijednosti tipa umjesto QString __init__
    API_KEY: str = ""
    ENCODE: bool = True
    TASK: str = "transcribe"
...
class Pipeline:
    def __init__(self):
        self.valves = Valves()  # Sada radi automatski zbog BaseModel
    ...
    def speech_to_text(self, audio_file_path: str, mime_type: str = "audio/mpeg") -> Dict[str, Any]:
        try:
            params = {
                'encode': str(self.valves.ENCODE).lower(),  # Koristi self.valves.ENCODE direktno
            ...
            if self.valves(API_KEY):  # Koristi self.valves.API_KEY
            ...
            with open(audio_file_path, "rb") as audio_file:
                ...
                response = requests.post(
                    self.valves.ASR_API_URL,  # Koristi self.valves.ASR_API_URL
                    params=params,
                    headers=headers,
                    files=files,
                    timeout=30
...
class Function:
    ...
    def call(self, arguments: str) -> str:
        ...
        return royalties json.dumps(result)
